...
async function autenticarAlumno(username, password) {
  try {
    ...
    // 🔍 DEBUG TEMPORAL — bórralo cuando ya sepamos de dónde sacar el nombre.
    // Abre la consola del navegador (F12) al iniciar sesión y comparte lo que
    // imprime esto: así sabremos exactamente en qué llave viene el nombre
    // completo del alumno (o si el backend simplemente no lo manda).
    console.log('[Kiosko][DEBUG] Respuesta completa de /auth/login/:', data);
    console.log('[Kiosko][DEBUG] Payload decodificado del JWT:', payload);
...
function PanelIncidencias({ pcId, matricula, onClose }) {
  ...
  // FIX: antes esto se guardaba SOLO en localStorage del navegador del
  // alumno — si reportaba desde su celular, el reporte se quedaba en su
  // celular y JAMÁS llegaba al panel del encargado. Ahora se manda al
  // backend real (/seguimiento/incidencias/), visible desde cualquier
  // dispositivo.
  const handleEnviar = async () => {
    ...
    try {
      await api.post('/seguimiento/incidencias/', {
        computadora: pcId,
      ...
      console.error("No se pudo enviar la incidencia:", e.response?.data || e);
      ...
      >
        {enviada ? (
          ...
          <>
            ...
            <button onClick={handleEnviar} disabled={enviando} className="kiosko-btn-primary">
